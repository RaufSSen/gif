  ...
  if (!message.channel.hasOwnProperty('poll')) {
    ...
    let pollStatus = await message.channel.send({
      embed: {
        ...
        title: 'Anket Başladı',
        description: `Anket ${message.author} tarafından başladı.\n\n**${message.channel.poll.message[0]}**`,
        fields: answers,
        footer: {
          text: `Numarayı söyliyerek oy verebilirsiniz. • Anket ${args.time} dakika içinde bitiyor.`
        }
      }
    });
    ...
    message.channel.poll.collector.on('collect', (msg, votes) => {
      ...
      msg.channel.send({
        embed: {
          description: `Teşekkürler, ${msg.author}, oyun verildi.`,
          footer: {
            text: `${votes.collected.size} toplam oy.`
          }
        }
      }).then(m => {
    ...
    message.channel.poll.collector.on('end', (pollRes) => {
      ...
      if (pollRes.length === 0) {
        return message.channel.send({
          embed: {
            ...
            title: 'Anket Bitti',
            description: 'Mağalesef kimse oy vermedi.'
          }
        }).then(() => {
      ...
      message.channel.send({
        embed: {
          color: Bastion.colors.BLUE,
          title: 'Anket bitti',
          description: `Anketin sonuçları **${message.channel.poll.message[0]}**`,
          fields: result
        }
      }).then(() => {
        pollStatus.delete().catch(e => {
...
exports.help = {
  ...
  userVoicePermission: '',
  usage: 'anket <soru>;<seçenek1>[;<seçenek2>[...]] [-t Dakika]',
  example: [ 'anket Dünyanın en iyi insanı?;Başkan;Sessiz2' ]
};

...
exports.exec = async (Bastion, message, args) => {
  ...
  if (userModel && userModel.dataValues.info) {
    ...
    info = ` ${user.id === message.author.id ? 'Bilgi ayarlanmamış `bilgiAyarla` komudu ile ayarlayabilirsin.' : ''}`;
  }
  ...
  let profileData = [
    ...
    {
      name: `🚧 İlerleme - ${totalRequiredExp.currentLevel} / ${totalRequiredExp.nextLevel} - ${Math.round(progress)}%`,
  ...
  if (userModel && userModel.dataValues.location) {
    profileData.push({
      name: '🏁 Yer',
      value: `${userModel.dataValues.location} yaşıyor.`,
      inline: true
    });
  }
...
exports.help = {
  ...
  example: [ 'profil', 'profil @DENEME#0001', 'profile 167433345337713651' ]
};

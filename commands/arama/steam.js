...
exports.exec = async (Bastion, message, args) => {
  ...
  if (!Object.keys(response).includes(args.category)) {
    return Bastion.emit('error', '', Bastion.i18n.error(message.guild.language, 'notFound', 'kategori'), message.channel);
  }
  ...
  await message.channel.send({
    embed: {
      ...
      footer: {
        text: `Sayfa: ${i + 1} of ${noOfPages > parseInt(noOfPages) ? parseInt(noOfPages) + 1 : parseInt(noOfPages)} • Steam tarafından Güçlendirildi`
      }
    }
  });
};

...
exports.exec = async (Bastion, message, args) => {
  ...
  if (message.author.id === message.guild.ownerID) {
    ...
    await message.channel.send({
      embed: {
        color: Bastion.colors.GREEN,
        description: `**${args.amount}** TL'ni <@${user.id}> kullanıcısına gönderdin.`
      }
    }).catch(e => {
      Bastion.log.error(e);
    });
    ...
    await user.send({
      embed: {
        color: Bastion.colors.GREEN,
        description: `Hesabına, **${message.guild.name}** Sunucusunda, **${args.amount}** TL hediye edildi.`
      }
    }).catch(e => {
      Bastion.log.error(e);
    });
    ...
    await message.author.send({
      embed: {
        ...
        description: `Hesabından, **${message.guild.name}** Sunucusunda, **${args.amount}** TL hediye edildi.`
      }
    }).catch(e => {
      Bastion.log.error(e);
    });

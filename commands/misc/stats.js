  ...
  await message.channel.send({
    embed: {
      ...
      fields: [
        ...
        {
          name: `Sahip {Bastion.credentials.ownerId.length > 1 ? 's' : ''}`,
          value: owners.join('\n') || '-',
          inline: true
        },
        {
          name: `Sahip ID${Bastion.credentials.ownerId.length > 1 ? 's' : ''}`,
          value: Bastion.credentials.ownerId.join('\n'),
          inline: true
        },
        {
          name: 'Öntanımlı Prefixler',
          value: Bastion.configurations.prefix.join(' '),
          inline: true
        },
        {
          name: 'Açık Durma(daha iyi olması için 5dkda bir reset)',
          value: uptime,
          inline: true
        },
        {
        ...
        {
          name: 'Shard Bilgileri',
          value: shardStats,
          inline: true
        },
        {
          name: 'Presence',
          value: `${guilds.toHumanString()} Sunucu\n`
          + `${textChannels.toHumanString()} Yazı Kanalı\n`
          + `${voiceChannels.toHumanString()} Ses Kanalı`,
          inline: true
        },
        {
          name: 'Hafuza',

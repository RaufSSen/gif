...
module.exports = async message => {
  try {
    ...
    if (message.client.configurations.logLevel === 1) {
      ...
      message.client.log.console(COLOR`{green [   SUNUCU]:} ${message.guild.name} / {cyan ${message.guild.id}}`);
      message.client.log.console(COLOR`{green [    KANAL]:} ${message.channel.name} / {cyan ${message.channel.id}}`);
      message.client.log.console(COLOR`{green [KULLANICI]:} ${message.author.tag} / {cyan ${message.author.id}}`);
      message.client.log.console(COLOR`{green [    KOMUT]:} {yellow ${usedPrefix}${command}} ${args.join(' ')}`);
    }
    else {
      message.client.log.console(`\n[${new Date()}]`);
      if (message.client.shard) {
      ...
      message.client.log.console(COLOR`{green [    KANAL]:} ${message.channel.name} / {cyan ${message.channel.id}}`);
      message.client.log.console(COLOR`{green [KULLANICI]:} ${message.author.tag} / {cyan ${message.author.id}}`);
      message.client.log.console(COLOR`{green [    KOMUT]:} ${usedPrefix}${command} / {cyan ${mdl}}`);
      message.client.log.console(COLOR`{green [  ARGÜMAN]:} ${args.join(' ') || COLOR`{yellow Çalıştıralacak argüman yok.}`}`);
    }

...
*/
    ...
    if (!Bastion.shard || process.env.SHARDS_READY) {
      ...
      Bastion.log.console(COLOR`\n{gray </> DeJavu ❤ ile yapmıştır}`);

      Bastion.log.console(COLOR`\n{cyan [${Bastion.user.username}]:} Uçmaya hazırım! 🚀\n`);

      if (Bastion.shard) {
        Bastion.log.console(COLOR`{green [   SHARDS]:} ${Bastion.shard.count}`);
      }
      Bastion.log.console(COLOR`{green [SUNUCULAR]:} ${guilds}`);
      Bastion.log.console(COLOR`{green [   PREFIX]:} ${Bastion.configurations.prefix.join(' ')}`);
      Bastion.log.console(COLOR`{green [ KOMUTLAR]:} ${Bastion.commands.size}`);
      Bastion.log.console(COLOR`{green [  BAŞLAMA]:} ${bootTime}ms`);

      Bastion.webhook.send('bastionLog', {
        color: Bastion.colors.BLUE,
        title: 'Kullanılmaya Hazırım!  🚀',
